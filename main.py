...
def analyze_coin(coin: str):
    ...
    rsi_long_ok = 50 <= rsi_15m <= 68
    rsi_short_ok = 32 <= rsi_15m <= 50
    ...
    if score_long >= MIN_SIGNAL_SCORE and (breakout_long or reclaim_long):
        ...
        if risk > 0:
            ...
            if rr >= RISK_REWARD_MIN:
                trigger = "Breakout" if breakout_long else "Sweep + Reclaim"
                return {
                    "coin": coin,
                    "side": "LONG",
                    "entry": entry,
    ...
    if score_short >= MIN_SIGNAL_SCORE and (breakdown_short or reject_short):
        ...
        if risk > 0:
            ...
            rr = (entry - tp1) / risk
            if rr >= RISK_REWARD_MIN:
                trigger = "Breakdown" if breakdown_short else "Sweep + Reject"
                return {
                    "coin": coin,
                    "side": "SHORT",
                    "entry": entry,
...
def main():
    ...
    state = load_state()
    post_discord("✅ Safe Pro Signal Bot gestartet. BTC und SOL werden überwacht.")
    ...
    while True:
        try:
            ...
            try:
                post_discord(f"⚠️ Bot-Fehler: {str(e)[:1200]}")
            except Exception:
                pass
