...
def analyze_coin_of_day(coin, market_ctx, prev_ctx, market_bias, flip_state, regime):
    ...
    e20_1h = ema(closes1, 20)
    atr15 = atr(c15, 14)
    if not e20_15 or not e50_15 or not e20_1h or atr15 is None:
        return None
    ...
    score = 0
    reasons = []
    side = "LONG" if price > e20_15 and e20_15 > e50_15 and closes1[-1] > e20_1h else \
           "SHORT" if price < e20_15 and e20_15 < e50_15 and closes1[-1] < e20_1h else "NEUTRAL"
...
def main():
    ...
    if SEND_STARTUP_MESSAGE:
        safe_discord(state, "✅ V12 ELITE BOT AKTIV", dedupe_key="startup")
        ...
        try:
            ...
            for coin in COINS:
                ...
                if cod.get("date") == now_utc().strftime("%Y-%m-%d") and cod.get("coin") == coin:
                    sig["score"] += COIN_OF_DAY_PRIORITY_BOOST
            ...
            if candidates:
                candidates.sort(key=lambda x: (x["score"], x["rr"], x["coin"] == state.get("coin_of_day", {}).get("coin", "")), reverse=True)
