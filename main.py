...
if SETUP_MODE == "SAFE":
    MIN_SIGNAL_SCORE_A = 8
    MIN_SIGNAL_SCORE_B = 6
    RISK_REWARD_MIN_A = 2.0
    RISK_REWARD_MIN_B = 1.6
elif SETUP_MODE == "AGGRESSIVE":
    MIN_SIGNAL_SCORE_A = 6
    MIN_SIGNAL_SCORE_B = 4
    RISK_REWARD_MIN_A = 1.4
    RISK_REWARD_MIN_B = 1.2
else:
    MIN_SIGNAL_SCORE_A = 7
    MIN_SIGNAL_SCORE_B = 5
    RISK_REWARD_MIN_A = 1.8
    RISK_REWARD_MIN_B = 1.4
...
def maybe_send_heartbeat(state):
    ...
    if now.hour == HEARTBEAT_HOUR_UTC and state.get("last_heartbeat_date") != today:
        safe_discord("💓 Bot läuft. Anti-Cluster Risk Control aktiv.")
...
def register_loss(state):
    ...
    if state["loss_streak"] >= MAX_LOSS_STREAK:
        ...
        safe_discord(f"⏸ **Bot Pause aktiviert** nach {state['loss_streak']} SLs in Folge bis {pause_until.strftime('%Y-%m-%d %H:%M UTC')}")
    save_state(state)
        ...
        if risk > 0:
            ...
            if score_long >= MIN_SIGNAL_SCORE_A and rr >= RISK_REWARD_MIN_A:
                candidates.append({
                    ...
                    "grade": "A",
                    "trigger": trigger,
                    "reason": "Trend, reclaim/breakout bestätigt, Funding/OI passen.",
                    "funding": funding,
                ...
                candidates.append({
                    ...
                    "grade": "B",
                    "trigger": trigger,
                    "reason": "Ordentliches Long-Setup.",
                    "funding": funding,
                    "open_interest": open_interest,
                    "mark_px": mark_px,
                    "risk_pct": risk_pct_for_grade(score_long),
        ...
        if risk > 0:
            ...
            if score_short >= MIN_SIGNAL_SCORE_A and rr >= RISK_REWARD_MIN_A:
                candidates.append({
                    ...
                    "grade": "A",
                    "trigger": trigger,
                    "reason": "Trend, reject/breakdown bestätigt, Funding/OI passen.",
                    "funding": funding,
                    "open_interest": open_interest,
                    "mark_px": mark_px,
                ...
                candidates.append({
                    ...
                    "grade": "B",
                    "trigger": trigger,
                    "reason": "Ordentliches Short-Setup.",
                    "funding": funding,
                    "open_interest": open_interest,
                    "mark_px": mark_px,
                    "risk_pct": risk_pct_for_grade(score_short),
...
def main():
    ...
    try:
        ...
        if SEND_STARTUP_MESSAGE:
            safe_discord("✅ Risk-Control Bot gestartet. Anti-Cluster Modus aktiv.")
        ...
        while True:
            try:
                ...
                if not candidates:
                    print("No valid setup after risk filters", flush=True)
                    time.sleep(POLL_SECONDS)
                    continue
