...
# Signalqualität
RISK_REWARD_MIN_A = float(os.getenv("RISK_REWARD_MIN_A", "1.8"))
RISK_REWARD_MIN_B = float(os.getenv("RISK_REWARD_MIN_B", "1.4"))
MIN_SIGNAL_SCORE_A = int(os.getenv("MIN_SIGNAL_SCORE_A", "7"))
...
LOW_QUALITY_DAY_LIMIT = int(os.getenv("LOW_QUALITY_DAY_LIMIT", "4"))
NORMAL_DAY_LIMIT = int(os.getenv("NORMAL_DAY_LIMIT", "7"))
HIGH_QUALITY_DAY_LIMIT = int(os.getenv("HIGH_QUALITY_DAY_LIMIT", "10"))
...
def daily_limit_ok(state, current_limit):
    today = datetime.now(timezone.utc).strftime("%Y-%m-%d")
    if state["daily_count"]["date"] != today:
        state["daily_count"] = {"date": today, "count": 0}
        save_state(state)
    return state["daily_count"]["count"] < current_limit
...
def maybe_send_heartbeat(state):
    ...
    if now.hour == HEARTBEAT_HOUR_UTC and state.get("last_heartbeat_date") != today:
        safe_discord("💓 Bot läuft. Multi-Coin Scan + Smart Exit aktiv.")
        state["last_heartbeat_date"] = today
        save_state(state)


def send_status_report(state, market_bias):
...
def get_dynamic_daily_limit(candidates, market_bias):
    if not SMART_LIMIT_ENABLED:
        return MAX_SIGNALS_PER_DAY

    a_count = len([c for c in candidates if c["grade"] == "A"])
    b_count = len([c for c in candidates if c["grade"] == "B"])

    if a_count >= 2:
        return HIGH_QUALITY_DAY_LIMIT
    if a_count >= 1 or (b_count >= 3 and market_bias != "neutral"):
        return NORMAL_DAY_LIMIT
    return LOW_QUALITY_DAY_LIMIT


def analyze_coin(coin: str, market_ctx: dict, state: dict, market_bias: str):
        ...
        if risk > 0:
            ...
            if score_long >= MIN_SIGNAL_SCORE_A and rr >= RISK_REWARD_MIN_A:
                candidates.append({
                    ...
                    "grade": "A",
                    "trigger": trigger,
                    "reason": "Trend, fakeout/reclaim oder breakout bestätigt, Funding/OI passen.",
                    "funding": funding,
                ...
                candidates.append({
                    ...
                    "grade": "B",
                    "trigger": trigger,
                    "reason": "Ordentliches Long-Setup, aber nicht stark genug für Discord.",
                    "funding": funding,
                    "open_interest": open_interest,
                    "mark_px": mark_px,
                    "risk_pct": risk_pct_for_grade(score_long),
        ...
        if risk > 0:
            ...
            if score_short >= MIN_SIGNAL_SCORE_A and rr >= RISK_REWARD_MIN_A:
                candidates.append({
                    ...
                    "grade": "A",
                    "trigger": trigger,
                    "reason": "Trend, fakeout/reject oder breakdown bestätigt, Funding/OI passen.",
                    "funding": funding,
                    "open_interest": open_interest,
                    "mark_px": mark_px,
                ...
                candidates.append({
                    ...
                    "grade": "B",
                    "trigger": trigger,
                    "reason": "Ordentliches Short-Setup, aber nicht stark genug für Discord.",
                    "funding": funding,
                    "open_interest": open_interest,
                    "mark_px": mark_px,
                    "risk_pct": risk_pct_for_grade(score_short),
    ...
    for coin, trade in state["open_trades"].items():
        ...
        if side == "LONG":
            ...
            if trade["tp1_hit"] and price < ema20_15m:
                safe_discord(format_close_message(coin, trade, "Nach TP1 unter 15m EMA gefallen", price))
                trade["closed"] = True
                to_remove.append(coin)
                continue
        ...
        if side == "SHORT":
            ...
            if trade["tp1_hit"] and price > ema20_15m:
                safe_discord(format_close_message(coin, trade, "Nach TP1 über 15m EMA gestiegen", price))
                trade["closed"] = True
                to_remove.append(coin)
                continue
...
def main():
    ...
    try:
        ...
        if SEND_STARTUP_MESSAGE:
            safe_discord("✅ Safe Pro Signal Bot V7 gestartet. Multi-Coin Scan + Smart Exit aktiv.")
        ...
        while True:
            ...
            try:
                ...
                send_status_report(state, market_bias)
                ...
                for coin in COINS:
                    ...
                    if coin not in market_ctx:
                        print(f"{coin}: no market ctx", flush=True)
                        continue

                    if coin in state["open_trades"]:
                        print(f"{coin}: trade already open", flush=True)
                        continue

                    print(f"Checking {coin}", flush=True)
                    sig = analyze_coin(coin, market_ctx, state, market_bias)
                ...
                current_daily_limit = get_dynamic_daily_limit(candidates, market_bias)
                print(f"Dynamic daily limit: {current_daily_limit}", flush=True)

                if not daily_limit_ok(state, current_daily_limit):
                ...
                for c in candidates:
                    if c["grade"] == "B":
                        print(f"{c['coin']}: B-setup only ({c['trigger']})", flush=True)
                ...
                for sig in a_candidates:
                    ...
                    state_key = f"{sig['coin']}_{sig['side']}"
                    if not cooldown_ok(state, state_key):
                        print(f"{sig['coin']}: cooldown active for {sig['side']}", flush=True)
                        continue
