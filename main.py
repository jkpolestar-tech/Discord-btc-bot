...
MIN_SIGNAL_SCORE = int(os.getenv("MIN_SIGNAL_SCORE", "6"))   # nur A-Setups
...
HIGH_RISK_UTC_HOURS = {18, 19, 20, 21}  # grober Schutz für US-Makrozeiten
...
def analyze_coin(coin: str):
    ...
    # keine enge Seitwärtsphase
    not_chop = range_pct >= 0.85 and atr_pct >= 0.22

    # Preis nicht zu weit vom 15m EMA weg
...
def main():
    ...
    while True:
        try:
            ...
            if in_news_block():
                log("News block active. Skipping scan.")
                time.sleep(POLL_SECONDS)
                continue

            if not daily_limit_ok(state):
                log("Daily signal limit reached. Waiting.")
                time.sleep(POLL_SECONDS)
                continue
            ...
            for coin in COINS:
                ...
                if not sig:
                    log(f"{coin}: no A-setup")
                    continue

                state_key = f"{coin}_{sig['side']}"
                if not cooldown_ok(state, state_key):
                    log(f"{coin}: cooldown active for {sig['side']}")
                    continue

                msg = format_signal(sig)
                post_discord(msg)
                log(f"{coin}: signal sent -> {sig['side']} ({sig['trigger']})")
