...
MAX_LONG_FUNDING = float(os.getenv("MAX_LONG_FUNDING", "0.0007"))
MIN_SHORT_FUNDING = float(os.getenv("MIN_SHORT_FUNDING", "-0.0007"))

# ===== Structure / levels =====
LOOKBACK_BREAK = int(os.getenv("LOOKBACK_BREAK", "12"))
...
ENTRY_ON_RETEST_ONLY = os.getenv("ENTRY_ON_RETEST_ONLY", "true").lower() == "true"
...
def load_state():
    ...
    state.setdefault("daily_loss", {"date": "", "sl_count": 0, "pause_until": ""})
    return state
...
def ensure_daily_loss_state(state):
    ...
    if daily.get("date") != today:
        state["daily_loss"] = {"date": today, "sl_count": 0, "pause_until": ""}
...
def rejection_ok(c, side):
    if side == "LONG":
        return lower_wick_ratio(c) >= 0.25 or (c["c"] > c["o"] and candle_body_strength(c) >= 0.30)
    return upper_wick_ratio(c) >= 0.25 or (c["c"] < c["o"] and candle_body_strength(c) >= 0.30)
...
def anti_chase_ok(side, price, e20_15, atr15, high_break, low_break, c15):
    ...
    if side == "SHORT":
        ...
        if RETEST_REQUIRED_AFTER_BREAK:
            touched_retest = prev["c"] < low_break and last["h"] >= low_break - atr15 * RETEST_TOLERANCE_ATR
    ...
    if side == "LONG":
        ...
        if abs(last["c"] - last["o"]) > atr15 * 1.1 and last["c"] > high_break:
            return False
        if RETEST_REQUIRED_AFTER_BREAK:
            touched_retest = prev["c"] > high_break and last["l"] <= high_break + atr15 * RETEST_TOLERANCE_ATR
...
def retest_entry_ok(side, c15, break_level, atr15):
    if not ENTRY_ON_RETEST_ONLY:
        return True
    ...
    prev = c15[-2]
    last = c15[-1]
    tol = atr15 * RETEST_TOLERANCE_ATR

    if side == "SHORT":
        touched = last["h"] >= (break_level - tol)
        held_below = last["c"] < break_level
        reject = rejection_ok(last, "SHORT")
        return touched and held_below and reject

    touched = last["l"] <= (break_level + tol)
    held_above = last["c"] > break_level
    reject = rejection_ok(last, "LONG")
    return touched and held_above and reject
...
def get_market_regime():
    ...
    if closes1[-1] > ema(closes1, 20) > ema(closes1, 50):
        return "TREND_LONG"
    if closes1[-1] < ema(closes1, 20) < ema(closes1, 50):
        return "TREND_SHORT"
    return "RANGE"
...
def analyze(coin, market_ctx, prev_ctx, market_bias, flip_state, regime, state):
    ...
    if regime == "TREND_LONG":
        ...
        if bullish_sweep:
            side = "LONG"
            score += 3
            trigger = "Range Sweep Long"
        elif bearish_sweep:
            side = "SHORT"
            score += 3
            trigger = "Range Sweep Short"
        else:
            return None
    elif regime == "IMPULSE_LONG":
        if bullish_15m and bullish_1h:
            side = "LONG"
            score += 4
            trigger = "Impulse Continuation Long"
        else:
            return None
    elif regime == "IMPULSE_SHORT":
        if bearish_15m and bearish_1h:
            side = "SHORT"
            score += 4
            trigger = "Impulse Continuation Short"
        else:
            return None
    else:
        return None
    ...
    if SMART_MONEY_FILTER:
        if side == "LONG":
            ...
            if retest_entry_ok("LONG", c15, high_break, atr15):
                score += 2
                if trigger is None:
                    trigger = "Retest Long"
        else:
            ...
            if retest_entry_ok("SHORT", c15, low_break, atr15):
                score += 2
                if trigger is None:
                    trigger = "Retest Short"

    if trigger is None:
        trigger = "Institutional Long" if side == "LONG" else "Institutional Short"

    if side == "LONG":
        level_ok = fake_breakout_filter_ok("LONG", c15, high_break if breakout_long else (high_cluster or e20_15))
    else:
        level_ok = fake_breakout_filter_ok("SHORT", c15, low_break if breakdown_short else (low_cluster or e20_15))
    if not level_ok:
        return None
    ...
    if side == "LONG" and 42 < r < 74:
        score += 1
    if side == "SHORT" and 26 < r < 58:
        score += 1
    ...
    if REGIME_FILTER_ENABLED and regime == "CHAOS":
        return None
...
def main():
    ...
    if SEND_STARTUP_MESSAGE:
        safe_discord(state, "✅ V14 INSTITUTIONAL SMART-ENTRY BOT AKTIV", dedupe_key="startup")
    ...
    while True:
        try:
            ...
            if candidates:
                ...
                candidates.sort(key=lambda x: (x["coin"] == cod_coin, x["score"], x["rr"]), reverse=True)
