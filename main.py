...
STATUS_INTERVAL_MINUTES = int(os.getenv("STATUS_INTERVAL_MINUTES", "120"))
...
REQUIRE_BTC_ALIGNMENT_FOR_ALTS = os.getenv("REQUIRE_BTC_ALIGNMENT_FOR_ALTS", "false").lower() == "true"

USE_VOLUME_FILTER = os.getenv("USE_VOLUME_FILTER", "true").lower() == "true"
VOLUME_BOOST_MIN = float(os.getenv("VOLUME_BOOST_MIN", "1.05"))

USE_PULLBACK_ENTRIES = os.getenv("USE_PULLBACK_ENTRIES", "true").lower() == "true"
USE_BREAKOUT_ENTRIES = os.getenv("USE_BREAKOUT_ENTRIES", "true").lower() == "true"

ENTRY_CONFIRMATION = os.getenv("ENTRY_CONFIRMATION", "false").lower() == "true"
ENTRY_ZONE_PCT = float(os.getenv("ENTRY_ZONE_PCT", "0.0015"))
STOP_PCT = float(os.getenv("STOP_PCT", "0.018"))

BREAKOUT_LOOKBACK = int(os.getenv("BREAKOUT_LOOKBACK", "12"))
PULLBACK_LOOKBACK = int(os.getenv("PULLBACK_LOOKBACK", "8"))

BASE_MIN_SCORE = int(os.getenv("BASE_MIN_SCORE", "4"))
...
LONG_COOLDOWN_AFTER_SL_HOURS = int(os.getenv("LONG_COOLDOWN_AFTER_SL_HOURS", "3"))
SHORT_COOLDOWN_AFTER_SL_HOURS = int(os.getenv("SHORT_COOLDOWN_AFTER_SL_HOURS", "3"))
COIN_COOLDOWN_AFTER_SL_HOURS = int(os.getenv("COIN_COOLDOWN_AFTER_SL_HOURS", "4"))
...
COIN_OF_DAY_MIN_SCORE = int(os.getenv("COIN_OF_DAY_MIN_SCORE", "4"))
...
def entry_confirmation_ok(c15, side):
    ...
    prev = c15[-2]
    if side == "LONG":
        return last["c"] > last["o"] or last["c"] > prev["h"]
    return last["c"] < last["o"] or last["c"] < prev["l"]
...
def analyze(coin, market_ctx, prev_ctx, btc_bias, state):
    ...
    high_break = max(closes15[-BREAKOUT_LOOKBACK:-1])
    low_break = min(closes15[-BREAKOUT_LOOKBACK:-1])
    ...
    pullback_long = (
        USE_PULLBACK_ENTRIES and
        bullish_15m and bullish_1h and
        min(recent_lows) <= e20_15 * 1.002 and
        price > e20_15
    )

    pullback_short = (
        USE_PULLBACK_ENTRIES and
        bearish_15m and bearish_1h and
        max(recent_highs) >= e20_15 * 0.998 and
        price < e20_15
    )
    ...
    if USE_BREAKOUT_ENTRIES and bullish_15m and bullish_1h and breakout_long:
        side = "LONG"
        trigger = "Breakout Long"
        score += 3
    elif USE_BREAKOUT_ENTRIES and bearish_15m and bearish_1h and breakdown_short:
        side = "SHORT"
        trigger = "Breakout Short"
        score += 3
    elif pullback_long:
        side = "LONG"
    ...
    if side == "LONG" and 40 < r < 78:
        score += 1
    if side == "SHORT" and 22 < r < 60:
        score += 1
    ...
    if USE_VOLUME_FILTER:
        ...
        if ref_vol > 0 and current_vol >= ref_vol * VOLUME_BOOST_MIN:
            score += 1
        elif trigger.startswith("Breakout"):
            return None
    ...
    if REQUIRE_BTC_ALIGNMENT_FOR_ALTS and coin not in MAJORS:
        if btc_bias not in (side, "NEUTRAL"):
            return None
    ...
    if cod.get("date") == now_utc().strftime("%Y-%m-%d") and cod.get("coin") == coin:
    ...
    if side == "LONG":
        stop_ref = min([x["l"] for x in c15[-8:]])
        ...
        stop_ref = max([x["h"] for x in c15[-8:]])
...
def send_status_report(state, btc_bias):
    ...
    cod = state.get("coin_of_day", {})
    cod_text = cod.get("coin", "-") if cod.get("date") == now_utc().strftime("%Y-%m-%d") else "-"
    blocked = [c for c in COINS if is_coin_cooldown_active(state, c)]

    msg = (
        f"📊 **RECOVERY BOT STATUS**\n"
        f"BTC Bias: {btc_bias}\n"
...
def main():
    ...
    if SEND_STARTUP_MESSAGE:
        safe_discord(state, "✅ RECOVERY BOT AKTIV | Coin Cooldown aktiv", dedupe_key="startup")
