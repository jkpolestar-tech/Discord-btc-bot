...
MIN_SIGNAL_SCORE_B = int(os.getenv("MIN_SIGNAL_SCORE_B", "4"))
...
def make_entry_zone(entry, atr_value, side):
    pad = atr_value * 0.25
    if side == "LONG":
    ...
    if score_long >= MIN_SIGNAL_SCORE_B and (breakout_long or sweep_reclaim_long or failed_breakdown_long):
        ...
        if risk > 0:
            ...
            if score_long >= MIN_SIGNAL_SCORE_A and rr >= RISK_REWARD_MIN_A:
                ...
                ez = make_entry_zone(entry, atr_15m, "LONG")
                return {
                    "coin": coin,
                    "side": "LONG",
                    "entry": entry,
                    ...
                    "grade": "A",
                    "trigger": trigger,
                    "reason": "Trend stimmt, Fakeout/Sweep oder Break bestätigt, Funding/OI passen.",
                    "funding": funding,
                    "open_interest": open_interest,
                    "mark_px": mark_px,
                }

            if score_long >= MIN_SIGNAL_SCORE_B and rr >= RISK_REWARD_MIN_B:
                trigger = "Breakout + Hold" if breakout_long else ("Sweep + Reclaim" if sweep_reclaim_long else "Failed Breakdown Reclaim")
                return {
                    "coin": coin,
                    "side": "LONG",
                    "entry": entry,
                    "entry_zone": make_entry_zone(entry, atr_15m, "LONG"),
                    "stop": stop,
                    "tp1": tp1,
                    "tp2": tp2,
                    "rr": rr,
    ...
    if score_short >= MIN_SIGNAL_SCORE_B and (breakdown_short or sweep_reject_short or failed_breakout_short):
        ...
        if risk > 0:
            ...
            if score_short >= MIN_SIGNAL_SCORE_A and rr >= RISK_REWARD_MIN_A:
                ...
                ez = make_entry_zone(entry, atr_15m, "SHORT")
                return {
                    "coin": coin,
                    "side": "SHORT",
                    "entry": entry,
                    ...
                    "grade": "A",
                    "trigger": trigger,
                    "reason": "Trend stimmt, Fakeout/Sweep oder Breakdown bestätigt, Funding/OI passen.",
                    "funding": funding,
                    "open_interest": open_interest,
                    "mark_px": mark_px,
                }

            if score_short >= MIN_SIGNAL_SCORE_B and rr >= RISK_REWARD_MIN_B:
                trigger = "Breakdown + Hold" if breakdown_short else ("Sweep + Reject" if sweep_reject_short else "Failed Breakout Reject")
                return {
                    "coin": coin,
                    "side": "SHORT",
                    "entry": entry,
                    "entry_zone": make_entry_zone(entry, atr_15m, "SHORT"),
                    "stop": stop,
                    "tp1": tp1,
                    "tp2": tp2,
                    "rr": rr,
...
def main():
    ...
    try:
        ...
        if SEND_STARTUP_MESSAGE:
            safe_discord("✅ Safe Pro Signal Bot V5 gestartet. BTC und SOL werden überwacht.")
        ...
        while True:
            ...
            try:
                ...
                for coin in COINS:
                    ...
                    if sig["grade"] == "B":
                        print(f"{coin}: B-setup only ({sig['trigger']})", flush=True)
                        continue
