...
REQUIRE_FIRST_TRADE_PROFIT = os.getenv("REQUIRE_FIRST_TRADE_PROFIT", "true").lower() == "true"
...
BOS_BUFFER_ATR = float(os.getenv("BOS_BUFFER_ATR", "0.10"))
...
def avg_volume(candles, n=20):
    vals = [c["v"] for c in candles[-n:]]
    if not vals:
        return 0.0
    return sum(vals) / len(vals)


def volume_boost_ok(candles):
    if len(candles) < 25:
        return False
    current_v = candles[-1]["v"]
    avg_v = avg_volume(candles[:-1], 20)
    if avg_v <= 0:
        return False
    return current_v >= avg_v * VOLUME_BOOST_MIN


def entry_confirmation_ok(candles):
    if len(candles) < 3:
        return False
...
def detect_swings(candles, left_right=5):
    swing_highs = []
    swing_lows = []
    ...
    for i in range(left_right, len(candles) - left_right):
        ...
        is_high = all(h >= candles[j]["h"] for j in range(i - left_right, i + left_right + 1) if j != i)
        is_low = all(l <= candles[j]["l"] for j in range(i - left_right, i + left_right + 1) if j != i)
...
def get_structure_bias(candles):
    highs, lows = detect_swings(candles, SWING_LOOKBACK)
...
def detect_bos(candles, atr_value):
    if len(candles) < STRUCTURE_LOOKBACK:
        return "neutral"

    recent = candles[-STRUCTURE_LOOKBACK:]
    highs, lows = detect_swings(recent, SWING_LOOKBACK)

    if not highs or not lows or atr_value is None:
        return "neutral"

    last_close = recent[-1]["c"]
    prev_close = recent[-2]["c"]

    recent_swing_high = highs[-1][1]
    recent_swing_low = lows[-1][1]
    buffer = atr_value * BOS_BUFFER_ATR

    if prev_close <= recent_swing_high and last_close > recent_swing_high + buffer:
        return "bullish_bos"
    if prev_close >= recent_swing_low and last_close < recent_swing_low - buffer:
        return "bearish_bos"

    return "neutral"


def detect_sweep_signals(candles):
    if len(candles) < 30:
        return False, False
    ...
    local_high = max(c["h"] for c in recent)
    local_low = min(c["l"] for c in recent)
...
def maybe_send_heartbeat(state):
    ...
    if now.hour == HEARTBEAT_HOUR_UTC and state.get("last_heartbeat_date") != today:
        safe_discord("💓 Bot läuft. Structure + Sweep + Volume aktiv.")
...
def analyze_coin(coin: str, market_ctx: dict, state: dict, market_bias: str, market_regime: str):
    ...
    candles_15m = fetch_candles(coin, "15m", 180)
    ...
    if ENTRY_CONFIRMATION and not entry_confirmation_ok(candles_15m):
        return None
    ...
    structure_bias_15m = get_structure_bias(candles_15m[-80:])
    bos_signal = detect_bos(candles_15m[-80:], atr_15m)
    bullish_sweep, bearish_sweep = detect_sweep_signals(candles_15m[-60:])
    volume_ok = volume_boost_ok(candles_15m)
    ...
    if bullish_1h:
        score_long += 1
    if structure_bias_15m == "bullish":
        score_long += 1
    if bos_signal == "bullish_bos":
        score_long += 1
    if bullish_sweep or failed_breakdown_long:
        score_long += 1
    if volume_ok:
        score_long += 1
    if not_chop:
    ...
    if bearish_1h:
        score_short += 1
    if structure_bias_15m == "bearish":
        score_short += 1
    if bos_signal == "bearish_bos":
        score_short += 1
    if bearish_sweep or failed_breakout_short:
        score_short += 1
    if volume_ok:
        score_short += 1
    if not_chop:
    ...
    if score_long >= min_signal_score_b and (breakout_long or bullish_sweep or failed_breakdown_long or bos_signal == "bullish_bos"):
        entry = price
        ...
        if risk > 0:
            ...
            elif bos_signal == "bullish_bos":
                trigger = "Bullish BOS + Volume"
            elif breakout_long:
                trigger = "Breakout + Hold"
            else:
                trigger = "Failed Breakdown Reclaim"
            ...
            if score_long >= min_signal_score_a and rr >= risk_reward_min_a:
                candidates.append({
                    ...
                    "grade": "A",
                    "trigger": trigger,
                    "reason": f"{market_regime}-Regime, Structure/Sweep/Volume bestätigt, Funding/OI passen.",
                    "funding": funding,
                ...
                candidates.append({
                    ...
                    "grade": "B",
                    "trigger": trigger,
                    "reason": f"Long-Setup im {market_regime}-Regime.",
                    "funding": funding,
                    "open_interest": open_interest,
                    "mark_px": mark_px,
                    "risk_pct": risk_pct_for_grade(score_long),
    ...
    if score_short >= min_signal_score_b and (breakdown_short or bearish_sweep or failed_breakout_short or bos_signal == "bearish_bos"):
        entry = price
        ...
        if risk > 0:
            ...
            if bearish_sweep:
                ...
                trigger = "Bearish BOS + Volume"
            elif breakdown_short:
                trigger = "Breakdown + Hold"
            else:
                trigger = "Failed Breakout Reject"
            ...
            if score_short >= min_signal_score_a and rr >= risk_reward_min_a:
                ...
                candidates.append({
                    ...
                    "grade": "B",
                    "trigger": trigger,
                    "reason": f"Short-Setup im {market_regime}-Regime.",
                    "funding": funding,
                    "open_interest": open_interest,
                    "mark_px": mark_px,
                    "risk_pct": risk_pct_for_grade(score_short),
...
def passes_exposure_filters(sig, state, market_bias, btc_structure_ok):
    ...
    same_direction_trades = [
        t for t in state["open_trades"].values()
        if t["side"] == sig["side"]
    ]

    if len(same_direction_trades) >= MAX_TRADES_PER_DIRECTION:
        return False, "zu viele Trades gleiche Richtung"

    if REQUIRE_FIRST_TRADE_PROFIT and len(same_direction_trades) >= 1:
        first = same_direction_trades[0]
        if not first.get("tp1_hit"):
            return False, "erster Trade noch nicht bestätigt"
...
def main():
    ...
    try:
        ...
        if SEND_STARTUP_MESSAGE:
            safe_discord("✅ Structure Bot gestartet. Regime + Sweep + Volumen aktiv.")
        ...
        while True:
            try:
                ...
                if not candidates:
                    print("No valid setup after structure/sweep/volume/risk filters", flush=True)
                    time.sleep(POLL_SECONDS)
                    continue
