...
# Neu: Trend-Validierung nach Entry
...
def analyze_btc(market_ctx, prev_ctx):
    ...
    if pullback_long:
        ...
        score += 3
        invalidation_level = min(recent_lows)
    elif pullback_short:
        side = "SHORT"
        trigger = "Pullback Short"
        score += 3
        invalidation_level = max(recent_highs)
    else:
        return None
...
def trend_still_valid(trade):
    try:
        ...
        if e20_1h is None or e50_1h is None:
        ...
        if side == "LONG":
            if e20_1h <= e50_1h:
                return False, "1H EMA Trend verloren"
            ...
            if TREND_BREAK_USE_PULLBACK_INVALIDATION and invalidation_level and last_close < invalidation_level:
                return False, "unter Pullback-Tief geschlossen"
        ...
        if TREND_BREAK_USE_PULLBACK_INVALIDATION and invalidation_level and last_close > invalidation_level:
            return False, "über Pullback-Hoch geschlossen"
        return True, "Trend ok"
